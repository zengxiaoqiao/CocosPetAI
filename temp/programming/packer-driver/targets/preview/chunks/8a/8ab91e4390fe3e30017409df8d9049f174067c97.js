    ...
    execute: function () {
      ...
      _export("BtnMicroRecord", BtnMicroRecord = (_dec = ccclass('BtnMicroRecord'), _dec2 = property(Label), _dec3 = property(_crd && PetValue === void 0 ? (_reportPossibleCrUseOfPetValue({
        ...
        onLoad() {
          ...
          var btn = this.button || this.node.getComponent(Button);
        ...
        _ensureMicroHint() {
          ...
          var comp = this.node.getComponent('BtnMicroRandomText') || (this.btnMicro || this.node).getComponent('BtnMicroRandomText');
          this.microHint = comp;
        ...
        _ensureWaveform() {
          ...
          var n = this.recordingNode.getChildByName('mic_waveform');
        ...
        _syncWaveform(on, subdued) {
          if (subdued === void 0) {
            subdued = false;
          }

          var wf = this._ensureWaveform();
        ...
        _ensureNodes() {
          var root = this.btnMicro || this.node;
        ...
        _applyStopButtonsVisibility(visible) {
          for (var n of this._allStopButtons) {
        ...
        _applyState(state) {
          this._state = state;
          var root = this.btnMicro || this.node;
          var isPressingOnly = this._isPressing && state === MicroState.Ready;
          ...
          var showWave = state === MicroState.Recording || isPressingOnly;
        ...
        _applyRecordingNodeOpacity(opacity) {
          ...
          var op = this.recordingNode.getComponent(UIOpacity);
        ...
        _startRecordingVisuals(root) {
          ...
          var pulse = Math.max(0.02, this.recordingPulseScale);
          var up = new Vec3(this._recordingBaseScale.x * (1 + pulse), this._recordingBaseScale.y * (1 + pulse), 1);
          tween(root).to(0.45, {
            scale: up
          }, {
            easing: 'sineInOut'
        ...
        _applyDisabledOpacity(target, disabled) {
          var opacity = target.getComponent(UIOpacity);
        ...
        _pollNativeAsrManualOnce() {
          ...
          var elapsed = Date.now() - this._nativeAsrManualStopMs;

          var canFinalize = this._nativeAsrManualBestText.length > 0 && this._nativeAsrManualIdlePolls >= 4 && elapsed >= 500;
          ...
          if (sys.platform === sys.Platform.ANDROID && sys.isNative) {
            var dbg = ((_crd && NativeASR === void 0 ? (_reportPossibleCrUseOfNativeASR({
              error: Error()
            }), NativeASR) : NativeASR).pollDebug() || '').trim();

            if (dbg) {
              var parts = dbg.split(',');
              var err = parts.length >= 2 ? parseInt(parts[1], 10) : 0;

              if (err) {
                var _instance3;

                var msg = "\u8BC6\u522B\u5931\u8D25\uFF08" + err + "\uFF09"; // Common SpeechRecognizer errors:
        ...
        _showSentBriefly() {
          var sent = this.sentNode || this.sentStateNode;
        ...
        _sendToAI(text) {
          ...
          return _asyncToGenerator(function* () {
            try {
              var withTimeout = /*#__PURE__*/function () {
                var _ref = _asyncToGenerator(function* (p, ms) {
                  return yield Promise.race([p, new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), ms))]);
                });
              ...
              var reply = yield withTimeout((_crd && TokitChatService === void 0 ? (_reportPossibleCrUseOfTokitChatService({
                error: Error()
              }), TokitChatService) : TokitChatService).sendMessage(text), 60000);
              if (_this._stoppedByUser) return;

              _this._playVocalAndFinish(reply);
            } catch (e) {
              if (_this._stoppedByUser) return;
              console.warn('[BtnMicroRecord] AI chat failed', e);
              var msg = e instanceof Error && e.message === 'timeout' ? '没收到回复' : '呜…没发出';

              _this._handleNotSent(msg);
            }
          })();
        ...
        _playVocalAndFinish(text) {
          ...
          if (t) {
            ...
            var pv = this.petValue || (_crd && PetValue === void 0 ? (_reportPossibleCrUseOfPetValue({
              error: Error()
        ...
        _startAutoVoice() {
          ...
          return _asyncToGenerator(function* () {
            if (_this2._autoStarted) return;
            if (!_this2.autoVoice) return;

            if (!(_crd && NativeASR === void 0 ? (_reportPossibleCrUseOfNativeASR({
              error: Error()
            }), NativeASR) : NativeASR).isSupported()) {
              // Web auto-voice removed.
              _this2.showHint('仅真机支持语音');

              return;
            }

            _this2._autoStarted = true;

            _this2._startNativeAsrLoop();
          })();
        }
        ...
        _pollNativeAsrResult() {
          ...
          var text = ((_crd && NativeASR === void 0 ? (_reportPossibleCrUseOfNativeASR({
            error: Error()
          }), NativeASR) : NativeASR).pollResult() || '').trim();
