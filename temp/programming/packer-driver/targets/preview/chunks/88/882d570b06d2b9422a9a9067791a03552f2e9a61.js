      ...
      _export("PetInfoBar", PetInfoBar = (_dec = ccclass('PetInfoBar'), _dec2 = property(Label), _dec3 = property({
        ...
        constructor() {
          ...
          this._onFeedActionTap = () => {
            ...
            var toggle = (_find = find('Canvas/TogglePet')) == null ? void 0 : _find.getComponent(_crd && TogglePet === void 0 ? (_reportPossibleCrUseOfTogglePet({
              error: Error()
            }), TogglePet) : TogglePet);
        ...
        static isActivePetSleeping() {
          var isCat = sys.localStorage.getItem('petai_pet_choice') === 'cat';
          var node = find(isCat ? 'Canvas/cat' : 'Canvas/dog');
          if (!(node != null && node.active)) return false;
          var anim = node.getComponent(Animation);
          if (!anim) return false;
          var sleepClip = (isCat ? 'cat' : 'dog') + "03";
          var state = anim.getState(sleepClip);
        ...
        refreshLowHpFeedPrompt() {
          ...
          var text = (_crd && SharedBtnCounts === void 0 ? (_reportPossibleCrUseOfSharedBtnCounts({
            error: Error()
        ...
        _applyText() {
          ...
          var displayedInApp = this._getInAppDisplayText();
        ...
        _syncBarVisibility() {
          ...
          var hasText = !!((_this$textLabel = this.textLabel) != null && (_this$textLabel = _this$textLabel.string) != null && _this$textLabel.trim());
          var show = hasText || this._showingUserHint || this._showingPerMinuteLimitHint || this._feedPromptActive || this._sleepBubbleActive;
        ...
        _ensureBubbleChrome() {
          ...
          var legacyShadow = this.node.getChildByName('bubble_shadow');
          if (legacyShadow) legacyShadow.destroy();

          var native = this._getBubbleNativeSize();
        ...
        _getBubbleNativeSize() {
          ...
          var frame = (_this$_bubbleSprite = this._bubbleSprite) == null ? void 0 : _this$_bubbleSprite.spriteFrame;
          if (!frame) return {
            w: 0,
            h: 0
          };
        ...
        _ensureActionButton() {
          ...
          var action = new Node('feed_action');
          var uit = action.addComponent(UITransform);
          uit.setContentSize(this.actionIconWidth, this.actionIconWidth);
          uit.setAnchorPoint(0.5, 0.5);
          var sp = action.addComponent(Sprite);
          sp.sizeMode = Sprite.SizeMode.CUSTOM;
          var btn1 = find('Canvas/btn/Button1');
          var src = (btn1 == null ? void 0 : btn1.getComponent(Sprite)) || (btn1 == null ? void 0 : btn1.getComponentInChildren(Sprite));
          ...
          var btn = action.addComponent(Button);
        ...
        _applyHorizontalPaddingForText(text, withAction) {
          ...
          var actionExtra = withAction ? this.actionIconWidth + 12 : 0;
          var maxWidth = Math.max(120, visible.width * (1 - ratio * 2) - inset * 2 - actionExtra);
          var t = (text || '').trim();
          var fontSize = Math.max(12, this.textLabel.fontSize || 34);
          var wideCount = 0;
          var narrowCount = 0;
          ...
          if (labelTrans) {
            ...
            var bubbleW = Math.max(this.bubbleMinWidth, targetWidth + this.bubblePaddingX * 2 + actionExtra); // 纵向保持原图高度，避免九宫格把阴影挤糊（pet_chat 上下 cap 各约 50px）

            var bubbleH = Math.max(this.bubbleMinHeight, native.h || 0);
