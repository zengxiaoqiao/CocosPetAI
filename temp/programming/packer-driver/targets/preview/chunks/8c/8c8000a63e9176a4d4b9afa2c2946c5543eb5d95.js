  ...
  return {
    ...
    execute: function () {
      ...
      _export("PetButtons", PetButtons = (_dec = ccclass('PetButtons'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(ScrollView), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property({
        ...
        onLoad() {
          var swipeNode = this._getSwipeNode();
        ...
        _startSwipeAnimation() {
          ...
          if (this._isDogActive()) {
            ...
            var dogCtrl = (_this$dogNode = this.dogNode) == null ? void 0 : _this$dogNode.getComponent(_crd && DogController === void 0 ? (_reportPossibleCrUseOfDogController({
              error: Error()
            ...
            var catCtrl = (_this$catNode = this.catNode) == null ? void 0 : _this$catNode.getComponent(_crd && CatController === void 0 ? (_reportPossibleCrUseOfCatController({
              error: Error()
        ...
        _stopSwipeAnimation() {
          ...
          if (this._isDogActive()) {
            ...
            var dogCtrl = (_this$dogNode2 = this.dogNode) == null ? void 0 : _this$dogNode2.getComponent(_crd && DogController === void 0 ? (_reportPossibleCrUseOfDogController({
              error: Error()
            }), DogController) : DogController);
            ...
            var catCtrl = (_this$catNode2 = this.catNode) == null ? void 0 : _this$catNode2.getComponent(_crd && CatController === void 0 ? (_reportPossibleCrUseOfCatController({
              error: Error()
            }), CatController) : CatController);
        ...
        _onSwipeTouchMove(e) {
          var current = new Vec2();
          e.getUILocation(current);
          var deltaX = current.x - this._lastSwipePos.x;
          var deltaY = current.y - this._lastSwipePos.y;
          ...
          var currentDir = new Vec2(deltaX, deltaY);
          currentDir.normalize();

          if (!this._isTrackingSwipe) {
            var totalDistance = Math.sqrt((current.x - this._swipeTouchStart.x) * (current.x - this._swipeTouchStart.x) + (current.y - this._swipeTouchStart.y) * (current.y - this._swipeTouchStart.y));
            ...
            var dot = this._lastSwipeDirection.x * currentDir.x + this._lastSwipeDirection.y * currentDir.y;

            if (dot < -0.5) {
              this._swipeCount++;
              var dirStr = Math.abs(deltaX) > Math.abs(deltaY) ? deltaX > 0 ? '右' : '左' : deltaY > 0 ? '上' : '下';
        ...
        _onSwipeTouchEnd(e) {
          if (this._isTrackingSwipe) {
            var end = new Vec2();
            e.getUILocation(end);
            var totalDistance = Math.sqrt((end.x - this._swipeTouchStart.x) * (end.x - this._swipeTouchStart.x) + (end.y - this._swipeTouchStart.y) * (end.y - this._swipeTouchStart.y));
            ...
            if (totalDistance >= this.swipeThreshold) {
              ...
              var petNode = this._getCurrentPetNode();
