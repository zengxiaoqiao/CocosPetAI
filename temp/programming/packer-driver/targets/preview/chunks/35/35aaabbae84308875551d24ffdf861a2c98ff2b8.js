    ...
    execute: function () {
      ...
      _export("HeartBubbleAni", HeartBubbleAni = (_dec = ccclass('HeartBubbleAni'), _dec2 = property({
        ...
        _resolveHeartSpriteFrame() {
          ...
          var hi = (_find = find('Canvas/highintimate')) != null ? _find : this.node;
          var sp = hi == null ? void 0 : hi.getComponent(Sprite);
        ...
        _getBurstParent() {
          ...
          var dog = find('Canvas/dog');
          var cat = find('Canvas/cat');
        ...
        _resolveBurstCenterY(parent) {
          var petUIT = parent.getComponent(UITransform);
          if (!petUIT) return 300;
          var h = Math.max(petUIT.contentSize.height, 200);
          var ap = petUIT.anchorPoint.y;
          var visibleHeadY = h * (0.9 - ap);
        ...
        _spawnOneHeart() {
          ...
          var parent = this._getBurstParent();

          var bubble = new Node('HeartBubble');
          var uit = bubble.addComponent(UITransform);
          ...
          var startX = this.burstOffsetX;
          var startY = this._resolveBurstCenterY(parent) + this.burstOffsetY;
          var rise = this.riseMin + Math.random() * Math.max(0, this.riseMax - this.riseMin);
          var endX = startX + (Math.random() * 2 - 1) * this.spreadX;
          var endY = startY + rise;
          var s = this.heartScale * (0.92 + Math.random() * 0.16);
          var duration = this.flyDuration * (0.9 + Math.random() * 0.2);
          var peak = Math.min(255, Math.max(0, this.peakOpacity | 0));
          var fadeInTime = duration * 0.32;
          var fadeOutTime = duration - fadeInTime;
