      ...
      _export("SharedBtnCounts", SharedBtnCounts = {
        ...
        init() {
          try {
            ...
            var saved = sys.localStorage.getItem(STORAGE_KEY_BTN); // 首次安装（未设 first_run_done）强制 2/1/1，不读存档；非首次才从存档恢复
            ...
            var pendingStr = sys.localStorage.getItem(STORAGE_KEY_PENDING);
            this.pendingClaim = pendingStr === '1';
            var todayDate = (_crd && getLocalDateString === void 0 ? (_reportPossibleCrUseOfgetLocalDateString({
              error: Error()
            }), getLocalDateString) : getLocalDateString)();
            var yesterdayDate = (_crd && getYesterdayDateString === void 0 ? (_reportPossibleCrUseOfgetYesterdayDateString({
              error: Error()
            }), getYesterdayDateString) : getYesterdayDateString)();
            var lastCheckinDay = sys.localStorage.getItem(STORAGE_KEY_LAST_CHECKIN_DAY) || '';
            var lastClaimDate = sys.localStorage.getItem(STORAGE_KEY_LAST_CLAIM_DATE) || '';
            var isContinuous = lastClaimDate === yesterdayDate; // 每天最多弹一次：今天还没弹过则产生待领取；首次安装（未设 first_run_done）不弹 Check-in
        ...
        save() {
          try {
            ...
            var now = Date.now();
            var hourStart = Math.floor(now / MS_PER_HOUR) * MS_PER_HOUR;
        ...
        ensurePendingClaimWhenUserOpensCheckIn() {
          ...
          var yesterday = (_crd && getYesterdayDateString === void 0 ? (_reportPossibleCrUseOfgetYesterdayDateString({
            error: Error()
          }), getYesterdayDateString) : getYesterdayDateString)();
          var lastClaimDate = sys.localStorage.getItem(STORAGE_KEY_LAST_CLAIM_DATE) || '';
          ...
          try {
            ...
            var isContinuous = lastClaimDate === yesterday;
        ...
        getPendingRewardType() {
          ...
          var idx = sys.localStorage.getItem(STORAGE_KEY_PENDING_REWARD_INDEX);
          var n = idx !== null && idx !== '' ? parseInt(idx, 10) : -1;
        ...
        claim() {
          ...
          if (isContinuous) {
            ...
            var idxStr = sys.localStorage.getItem(STORAGE_KEY_PENDING_REWARD_INDEX);
            var which = idxStr !== null && idxStr !== '' ? parseInt(idxStr, 10) : -1;
            var whichSafe = which === 0 || which === 1 || which === 2 ? which : Math.floor(Math.random() * 3);
...
//# sourceMappingURL=8338cd5e6e374d0b2666a5978b7c5cf20fca046b.js.map