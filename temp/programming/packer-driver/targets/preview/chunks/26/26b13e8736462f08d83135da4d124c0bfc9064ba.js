System.register(["cc"], function (_export, _context) {
  ...
  function syncWidgetFromStorage() {
    try {
      ...
      var pet = sys.localStorage.getItem(STORAGE_KEY_PET) || 'dog';
      var hp = Math.min(MAX_VALUE, Math.max(0, parseInt(sys.localStorage.getItem(STORAGE_KEY_HP) || '500', 10) || 0));
      var moodRaw = (_sys$localStorage$get = sys.localStorage.getItem(STORAGE_KEY_MOOD)) != null ? _sys$localStorage$get : sys.localStorage.getItem(STORAGE_KEY_MOOD_LEGACY);
      var mood = Math.min(MAX_VALUE, Math.max(0, parseInt(moodRaw || '500', 10) || 0));
