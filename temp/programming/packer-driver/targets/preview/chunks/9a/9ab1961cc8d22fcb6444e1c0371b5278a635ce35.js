    ...
    execute: function () {
      ...
      _export("WidgetChoosePanel", WidgetChoosePanel = (_dec = ccclass('WidgetChoosePanel'), _dec2 = property(Node), _dec3 = property(_crd && HomePopupMask === void 0 ? (_reportPossibleCrUseOfHomePopupMask({
        ...
        _refreshPetVisibility() {
          ...
          var pet = (sys.localStorage.getItem(STORAGE_KEY_PET) || 'dog').toLowerCase();
          var isDog = pet === 'dog';
          var widgetS = root.getChildByName('widget_s');
          var widgetL = root.getChildByName('widget_l');

          if (widgetS) {
            var dogS = widgetS.getChildByName('dog_s');
            var catS = widgetS.getChildByName('cat_s');
            if (dogS) dogS.active = isDog;
            if (catS) catS.active = !isDog;
          }

          if (widgetL) {
            var dogL = widgetL.getChildByName('dog_l');
            var catL = widgetL.getChildByName('cat_l');
