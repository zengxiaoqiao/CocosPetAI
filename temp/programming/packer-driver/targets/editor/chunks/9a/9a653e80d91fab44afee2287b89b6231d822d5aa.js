      ...
      _export("PetInfoBar", PetInfoBar = (_dec = ccclass('PetInfoBar'), _dec2 = property(Label), _dec3 = property({
        ...
        constructor(...args) {
            ...
            this._getBatteryStateAsync().then(state => {
              if (!state) {
                ...
                if (state.level < 0.2) {
                  ...
                  const arr = (_crd && getBatteryLowTexts === void 0 ? (_reportPossibleCrUseOfgetBatteryLowTexts({
                    error: Error()
          ...
          this._refreshNetworkTip = () => {
            this._getNetworkTypeAsync().then(type => {
              if (!type) {
                ...
                const arr = (_crd && getNetworkTipsNone === void 0 ? (_reportPossibleCrUseOfgetNetworkTipsNone({
                  error: Error()
          ...
          this._maybeUpdateGreeting = () => {
            ...
            const rules = (_crd && getTimeRules === void 0 ? (_reportPossibleCrUseOfgetTimeRules({
              error: Error()
            ...
            for (const rule of rules) {
              ...
              const text = (_rule$texts$idx = rule.texts[idx]) != null ? _rule$texts$idx : rule.texts[0];
        ...
        async _loadWeather() {
          const info = await (_crd && getCurrentWeatherByDeviceLocation === void 0 ? (_reportPossibleCrUseOfgetCurrentWeatherByDeviceLocation({
            error: Error()
          ...
          if (sys.platform === sys.Platform.ANDROID && sys.isNative) {
            try {
              ...
              if (nat != null && (_nat$reflection = nat.reflection) != null && _nat$reflection.callStaticMethod) {
                const raw = nat.reflection.callStaticMethod('com/cocos/game/AppActivity', 'getBatteryState', '()Ljava/lang/String;');
        ...
        _getNetworkTypeAsync() {
          ...
          if (sys.platform === sys.Platform.ANDROID && sys.isNative) {
            try {
              ...
              if (nat != null && (_nat$reflection2 = nat.reflection) != null && _nat$reflection2.callStaticMethod) {
                const raw = nat.reflection.callStaticMethod('com/cocos/game/AppActivity', 'getNetworkType', '()Ljava/lang/String;');
          ...
          const conn = (_ref = (_nav$connection = nav == null ? void 0 : nav.connection) != null ? _nav$connection : nav == null ? void 0 : nav.mozConnection) != null ? _ref : nav == null ? void 0 : nav.webkitConnection;
        ...
        _applyText() {
          ...
          const displayedInApp = this._getInAppDisplayText(forceWeather); // 是否为「只显示一次、约 2 秒」的短暂提示（仅 App 内）


          const isFirstOpenShort = displayedInApp === this._firstOpenText;
          const isGreetingShort = displayedInApp === this._greetingText;
          const isShortLivedInApp = isFirstOpenShort || isGreetingShort; // 消耗「仅 App」：首次打开。若队列中还有下一句指引，则接着显示下一句；否则清空。
        ...
        _applyHorizontalPaddingForText(text) {
          ...
          const t = (text || '').trim(); // Estimate content width, then clamp to maxWidth.
